...
-- NOTES: Semua Index dipastikan valid, yaitu tidak negatif dan tidak melebihi panjang list/Matrix (0 <= i <= length)
...
getMatrixElement m row col
    | isEmptyMatrix m = 0
    | row > 0 = getMatrixElement (tail m) (row-1) col
    | row == 0
    = (head m) !! col
