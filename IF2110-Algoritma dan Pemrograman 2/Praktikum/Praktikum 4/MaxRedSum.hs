...
isOneElmt p =
  not (isTreeEmpty p) && isTreeEmpty (left p) && isTreeEmpty (right p)
...
maxRedSum tree
  | isTreeEmpty tree = 0
  | isOneElmt tree = akar tree
  | otherwise
  = let case1 = (akar tree) + (if isExistLeft tree then maxRedSum (left (left tree)) + maxRedSum (right (left tree)) else 0) + (if isExistRight tree then maxRedSum (left (right tree)) + maxRedSum (right (right tree)) else 0)
        case2 = maxRedSum (left tree) + maxRedSum (right tree)
    in max case1 case2
