...
depth p =
  if (isTreeEmpty p) then
    ...
    if (isOneElmt p) then
      ...
      1 + max (depth(right p)) (depth(left p))
...
sumDeepestLeaves p
  ...
  | otherwise
  = if depth (left p) > depth (right p) then sumDeepestLeaves (left p)
    else if depth (left p) < depth (right p) then sumDeepestLeaves (right p)
         else sumDeepestLeaves (left p) + sumDeepestLeaves (right p)
