...
int isPredikat(const Word w) {
 ...
 int isObjToken(Word w, int *hasTrailingComma) {
    *hasTrailingComma = 0;
    if (isCommaToken(w) || isSubjek(w) || isPredikat(w))
        return 0;
 ...
 int cfg_parser() {
    ...
    while (!EndWord) {
        if (currentState == S_EXPECT_SUBJECT) {
        ...
        else if (currentState == S_EXPECT_PREDICATE) {
        ...
        else if (currentState == S_EXPECT_FIRST_OBJECT) {
            if (isObjToken(currentWord, &hasComma)) {
                object++;
                currentState = hasComma ? S_EXPECT_OBJECT_AFTER_COMMA : S_EXPECT_SEPARATOR_OR_END;
            }
            else {
                return 0;
            }
        }
        else if (currentState == S_EXPECT_SEPARATOR_OR_END) {
        ...
        else if (currentState == S_EXPECT_OBJECT_AFTER_COMMA) {
            if (isObjToken(currentWord, &hasComma)) {
                object++;
                currentState = hasComma ? S_EXPECT_OBJECT_AFTER_COMMA : S_EXPECT_SEPARATOR_OR_END;
            }
    ...
    if (currentState == S_EXPECT_SEPARATOR_OR_END && object > 0) {
        return 1;
    } else {
        return 0;
    }
