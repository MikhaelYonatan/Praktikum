...
int height(BinTree P) {
    if (P == NULL) return 0;
    int hLeft = height(LEFT(P));
    int hRight = height(RIGHT(P));
    return (hLeft > hRight ? hLeft : hRight) + 1;
}

long long getSumOfLevel(BinTree P, int targetLevel) {
    if (P == NULL) {
        return 0;
    }
    
    if (targetLevel == 1) {
        return (long long)ROOT(P);
    }
    
    return getSumOfLevel(LEFT(P), targetLevel - 1) + 
           getSumOfLevel(RIGHT(P), targetLevel - 1);
}

int levelMaxSum(BinTree root) {
    if (root == NULL) return 0;

    int h = height(root);
    long long globalMaxSum = getSumOfLevel(root, 1);
    int maxLevel = 1;

    for (int i = 1; i <= h; i++) {
        
        long long currentSum = getSumOfLevel(root, i);
