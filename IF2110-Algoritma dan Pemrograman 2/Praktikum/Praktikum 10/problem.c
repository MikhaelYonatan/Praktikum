...
int countGoodNodesHelper(BinTree p, int maxSoFar) {
    ...
    if (isTreeEmpty(p)) {
        ...
        if (ROOT(p) >= maxSoFar) {
            int maxSoFar = ROOT(p);
            return 1 + countGoodNodesHelper(RIGHT(p), maxSoFar) + countGoodNodesHelper(LEFT(p), maxSoFar);
        }
        else {
            return countGoodNodesHelper(RIGHT(p), maxSoFar) + countGoodNodesHelper(LEFT(p), maxSoFar);
        }
    }
}
...
int countGoodNodes(BinTree p) {
    ...
    if (isTreeEmpty(p)) {
        ...
        return 1 + countGoodNodesHelper(RIGHT(p), ROOT(p)) + countGoodNodesHelper(LEFT(p), ROOT(p));
    }
}